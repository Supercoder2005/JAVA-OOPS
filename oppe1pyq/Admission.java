...
Problem Statement
 ...
 input.
 • Class Student has/should have the following members.– Private instance variables String name and String[] courses to store name
 and courses chosen respectively– Define required constructor(s)– Accessor methods getName( ) and getCourses(int) to get the name of the
 student and the course at a specific index respectively.– Mutator methods setName(String) and setCourses(int,String) to set the
 name of the student and the course at a specific index respectively.
 • Class Admission has method main that does the following.– Two objects of Student s1 and s2 are created. s2 is created using s1– name of Student s2 and second course chosen by s2 are updated by taking the
 input– Finally, name of s1, s2 and second course chosen by s1 and s2 are printed
...
class Student{
    ...
    public Student(Student s1){
        this.name = s1.name;
        int n = s1.courses.length;
        this.courses = new String[n];
        for(int i=0;i<n;i++){
            this.courses[i] = s1.courses[i];
        }
    }
